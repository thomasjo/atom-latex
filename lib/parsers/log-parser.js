...
const OUTPUT_PATTERN: RegExp = new RegExp('' +
...
// Error pattern
const ERROR_PATTERN: RegExp = new RegExp('' +
...
const INCOMPLETE_FONT_PATTERN: RegExp = /^LaTeX Font .*[^.]$/

export default class LogParser extends Parser {
  texFilePath: string
  projectPath: string

  constructor (filePath: string, texFilePath: string): void {
    super(filePath)
  ...
  parse (): ?LogParseResult {
    ...
    lines.forEach((line: string, index: number) => {
      ...
      let match: ?Array<string> = line.match(OUTPUT_PATTERN)
      if (match) {
        const filePath: string = match[1].replace(/"/g, '') // TODO: Fix with improved regex.
      ...
      if (match) {
        const lineNumber: ?number = match[2] ? parseInt(match[2], 10) : undefined
      ...
      if (match) {
        const lineNumber: ?number = match[4] ? parseInt(match[4], 10) : undefined
        result.messages.push({

...
const MAKEINDEX_ERROR_PATTERN = /^[*!]+ (.+) \(file = (.+), line = (\d+)\):$/
const MAKEINDEX_ERROR_MESSAGE_PATTERN = /^\s+--\s*(.*)$/
  ...
  parsers = [{
    ...
    messageParsers: [{
      ...
      parse: (match, index, lines) => {
        const messageMatch = lines[index + 1].match(MAKEINDEX_ERROR_MESSAGE_PATTERN)
        const lineNumber = parseInt(match[3])

        return {
          type: 'error',
          text: `${match[1]}: ${messageMatch[1]}`,
    ...
    messageParsers: [{
      ...
      parse: (match, index, lines) => {
        ...
        if (refMatch) {
          const lineNumber = parseInt(refMatch[1])

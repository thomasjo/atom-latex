...
export default class LogParser extends Parser {
  ...
  public parse() {
    ...
    lines.forEach((line, index) => {
      ...
      if (INCOMPLETE_FONT_PATTERN.test(line)) {
        match = (line + lines[index + 1].substring(15)).match(WARNING_INFO_PATTERN);
      } else {
        match = line.match(WARNING_INFO_PATTERN);
      }
      if (match) {

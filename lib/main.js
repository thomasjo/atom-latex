...
export default {
  ...
  consumeLinter (indieRegistry) {
    const myLinter = indieRegistry.register({name: 'LaTeX'})
    this.disposables.add(myLinter)
    // this.composer.setLinterIndie(myLinter)
    const LinterLogger = require('./loggers/linter-logger')
    global.latex.setLogger(new LinterLogger(myLinter))
  },

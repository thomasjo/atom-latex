...
export default class Composer {
  ...
  async build (rebuild) {
    ...
    return Promise.all(jobnames.map(jobname => this.buildJob(builder, rootFilePath, jobname, rebuild)))
      .catch(error => latex.log.error(error.message))
      .then(() => {
        this.destroyProgressIndicator()
        latex.log.groupEnd()
      })
  ...
  async buildJob (builder, rootFilePath, jobname, rebuild) {
    const statusCode = await builder.run(rootFilePath, jobname, rebuild)

...
export default class Composer extends Disposable {
  ...
  cacheBuildState (state) {
    ...
    if (this.cachedBuildStates.has(filePath)) {
      ...
      for (const subfile of oldState.getSubfiles().values()) {
    ...
    for (const subfile of state.getSubfiles().values()) {
  ...
  initializeBuildStateFromProperties (state, properties) {
    ...
    if (properties.jobnames) {
      ...
      state.setJobNames(typeof properties.jobnames === 'string' ? properties.jobnames.split(/[,\s]+/) : properties.jobnames)

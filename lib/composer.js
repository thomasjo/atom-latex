...
export default class Composer {
  ...
  async cleanJob (builder, rootFilePath, jobname) {
    ...
    for (const pattern of patterns) {
      ...
      if (path.isAbsolute(pattern)) {
        files = files.concat(glob.sync(absolutePattern))
      } else {
        files = files.concat(_.filter(generatedFiles, minimatch.filter(absolutePattern)))
      }
    }
  ...
  getCleanPatterns (builder, rootFilePath, jobname) {
    const { name } = path.parse(rootFilePath)
    ...
    return _.map(patterns, pattern => replaceProperitiesInString(pattern, properties))
  }
  ...
  getGeneratedFileList (builder, rootFilePath, jobname) {
    ...
    if (fdb) {
      return _.sortedUniq(_.concat(files, _.map(_.flatten(_.values(fdb)), file => path.resolve(dir, file))))
    }

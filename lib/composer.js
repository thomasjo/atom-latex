...
export default class Composer extends Disposable {
  ...
  initializeBuild (filePath, allowCached = false) {
    ...
    if (allowCached && this.cachedBuildStates.has(filePath)) {
      ...
      if (!this.initializeBuildStateFromProjectFile(state)) {
  ...
  initializeBuildStateFromProjectFile (state) {
    let [ projectPath, relativePath ] = atom.project.relativizePath(state.getProjectPath())
    ...
    // latex.json file.
    while (relativePath !== '..') {
      const filePath = path.resolve(projectPath, relativePath, 'latex.yaml')
      if (fs.existsSync(filePath)) {
        return this.doInitializeBuildStateFromProjectFile(state, filePath)
      }
  ...
  doInitializeBuildStateFromProjectFile (state, filePath) {
    try {
      ...
      if (config.root) {
        state.setFilePath(path.resolve(rootPath, config.root))
      }
      this.initializeBuildStateFromProperties(state, config)

      const name = path.relative(rootPath, state.getFilePath())
      let fileSection = config[name]

      if (fileSection) {
        if (fileSection.root) {
          state.setFilePath(path.resolve(rootPath, fileSection.root))
          // use properties from root section
          fileSection = config[fileSection.root]
        }
        this.initializeBuildStateFromProperties(state, fileSection)
      }
    } catch (error) {
      latex.log.error(`Parsing of latex.yaml failed: ${error.message}`)
      return false
    }

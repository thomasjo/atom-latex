...
export default class Composer extends Disposable {
  ...
  async build (shouldRebuild) {
    ...
    if (atom.config.get('latex.enableDicy')) {
      return await this.runDicy(['load', 'build', 'log', 'save'], { ignoreCache: shouldRebuild }, this.shouldOpenResult(), 'LaTeX Build')
    }
  ...
  async sync () {
    ...
    if (atom.config.get('latex.enableDicy')) {
      return await this.runDicy(['load'])
    }
  ...
  async clean (deepClean = false, startGroup = true) {
    ...
    if (atom.config.get('latex.enableDicy')) {
      return await this.runDicy(['load', 'clean', 'save'], { deepClean }, false, 'LaTeX Clean')
    }

    const { builder, state } = this.initializeBuild(filePath, true)
    if (!builder) return false

    if (startGroup) latex.log.group('LaTeX Clean')

    const jobs = state.getJobStates().map(jobState => this.cleanJob(builder, jobState, deepClean))

    await Promise.all(jobs)

    if (startGroup) latex.log.groupEnd()
  }

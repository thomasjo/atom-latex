...
export default class Composer extends Disposable {
  ...
  async startDiCy (setOptions = false) {
    if (!this.dicy) {
      setOptions = true
    ...
    if (setOptions) {
      ...
      await dicy.updateUserOptions(this.getDiCyOptions())
    }
  }
  ...
  async initializeDiCy (filePath, shouldRebuild = false, fastLoad = false) {
    ...
    await dicy.setInstanceOptions(filePath, options)
  ...
  async runDiCy (commands, { shouldRebuild, fastLoad, openResults, clearLog } = { shouldRebuild: false, fastLoad: false, openResults: true, clearLog: false }) {
    ...
    const sourcePath = this.initializeDiCy(filePath)

    if (clearLog) latex.log.clear()
    if (!fastLoad) latex.status.setBusy()

    const success = await dicy.run(sourcePath, commands)

    if (openResults && success) {
      const targets = await dicy.getTargetPaths(filePath, true)

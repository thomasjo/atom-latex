...
export default class Composer extends Disposable {
  ...
  async createOutputTree (state) {
    let outputDirectory = state.getOutputDirectory()
    if (!outputDirectory) return

    const projectPath = state.getProjectPath()
    outputDirectory = path.resolve(state.getProjectPath(), outputDirectory)
    const outputDirectoryWithTrailingSep = outputDirectory + path.sep

    await fsep.copy(projectPath, outputDirectory, {
      filter: filePath =>
        fs.lstatSync(filePath).isDirectory() &&
        filePath !== outputDirectory &&
        !filePath.startsWith(outputDirectoryWithTrailingSep)
    })
  }
  ...
  async build (shouldRebuild) {
    ...
    await this.createOutputTree(state)

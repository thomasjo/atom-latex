...
import * as dicy from '@dicy/client'
...
export default class Composer extends Disposable {
  ...
  constructor () {
    ...
    this.disposables.add(atom.config.onDidChange('latex.useDicy', () => this.exitDiCy()))
  }
  ...
  async startDiCy () {
    ...
    this.dicy = new dicy.Client()
    this.dicy.start()

    this.dicy.onLog((filePath, event) => {
      ...
      if (event.source) {
        message.filePath = path.resolve(rootPath, event.source.file)
      ...
      if (event.log) {
        message.logPath = path.resolve(rootPath, event.log.file)
        if (event.log.range) {
    ...
    this.dicy.onCommand((filePath, event) => {
      latex.log.info(`[${event.rule}] Executing \`${event.command}\``)
    })

    this.dicy.onFileDeleted((filePath, event) => {
      if (!event.virtual) {
  ...
  async initializeDiCy (filePath, shouldRebuild = false, fastLoad = false) {
    ...
    if (shouldRebuild) {
      ...
      await this.dicy.clear(filePath)
    }

    await this.dicy.setInstanceOptions(filePath, options)

    if (this.updateDiCyUserOptions) {
      // Update the options on thhis builder. This will set the options the user's
      // config file. All other builders will then reread the config file on their
      // next build.
      await this.dicy.updateOptions(filePath, this.getDiCyOptions(), true)
  ...
  async runDiCy (commands, { shouldRebuild = false, fastLoad = false, openResults = true, clearLog = false } = {}) {
    console.log(`${shouldRebuild}, ${fastLoad}, ${openResults}, ${clearLog}`)
    const { filePath, lineNumber } = getEditorDetails()
    const sourcePath = await this.initializeDiCy(filePath)

    if (clearLog) latex.log.clear()
    if (!fastLoad) latex.status.setBusy()

    const success = await this.dicy.run(sourcePath, commands)

    if (openResults && success) {
      const targets = await this.dicy.getTargetPaths(sourcePath, true)

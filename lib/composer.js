...
export default class Composer {
  ...
  getCleanPatterns (builder, rootFilePath, jobname) {
    ...
    const outputDir = atom.config.get('latex.outputDirectory')
    const properties = {
      outputDir: outputDir ? outputDir + path.sep : '',
  ...
  getGeneratedFileList (builder, rootFilePath, jobname) {
    ...
    const outputDir = atom.config.get('latex.outputDirectory')
    const pattern = path.join(dir, outputDir, `${jobname || name}*`)
  ...
  checkCleanExtensions () {
    ...
    cleanPatterns = _.without(cleanPatterns, removedExtensions.map(extension => `**/*${extension}`))

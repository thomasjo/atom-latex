...
export default class Composer extends Disposable {
  ...
  async startDiCy () {
    ...
    this.dicy = new Client(true)

    this.dicy.on('log', (filePath, event) => {
      const rootPath = path.dirname(filePath)
      const messages = event.messages.map(dicyMessage => {
        ...
        if (dicyMessage.source) {
          message.filePath = path.resolve(rootPath, dicyMessage.source.file)
        ...
        if (dicyMessage.log) {
          message.logPath = path.resolve(rootPath, dicyMessage.log.file)
  ...
  async initializeDiCy (filePath, shouldRebuild = false, fastLoad = false) {
    ...
    if (this.updateDiCyUserOptions) {
      ...
      // next build.
      await this.dicy.setUserOptions(filePath, this.getDiCyOptions(), true)

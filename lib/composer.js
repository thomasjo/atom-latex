...
export default class Composer extends Disposable {
  ...
  initializeBuildStateFromLatexJson (state) {
    const latexJsonPath = path.join(state.projectPath, 'latex.json')
    if (!fs.existsSync(latexJsonPath)) return

    try {
      const latexJson = JSON.parse(fs.readFileSync(latexJsonPath))

      if (latexJson.jobnames) {
        state.jobnames = latexJson.jobnames
      } else if (latexJson.jobname) {
        state.jobnames = [latexJson.jobname]
      }

      if (latexJson.program) {
        state.engine = latexJson.program
      }

      if (latexJson.format) {
        state.outputFormat = latexJson.format
      }

      if (latexJson.output_directory) {
        state.outputDirectory = latexJson.output_directory
      }

      if (latexJson.producer) {
        state.producer = latexJson.producer
      }
    } catch (error) {
  ...
  async buildJob (builder, state) {
    try {
      ...
      if (statusCode > 0 || !state.outputFilePath) {

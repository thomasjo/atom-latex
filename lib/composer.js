    ...
    return new Promise(async (resolve, reject) => {
            ...
            _.map(entries,
              (entry) => {
                return {
                  ...
                  filePath: this.alterParentPath(rootFilePath, entry.filePath),
                  range: [[entry.lineNumber-1,0], [entry.lineNumber-1,255]]
                }
              }))
      ...
      try {
        for (const jobname of jobnames) {
          ...
          if (this.linterIndie) {
            createLinterMessages(jobname, 'warning', result.warning)

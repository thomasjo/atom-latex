...
export default class Composer {
  ...
  async build () {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const createLinterMessages = (jobname, type, entries) => {
        if (this.linterIndie) {
          linterMessages = _.concat(linterMessages,
            _.map(entries,
              (entry) => {
                let message = {
                  type: entry.type ? entry.type : type,
                  text: jobname ? '[' + jobname + '] ' + entry.message : entry.message,
                  filePath: entry.filePath ? this.alterParentPath(rootFilePath, entry.filePath) : rootFilePath
                }
                if (entry.logPosition) {
                  message.trace = [{
                    type: 'Trace',
                    text: 'LaTeX build log',
                    filePath: result.logFilePath,
                    range: [[entry.logPosition[0], 0], [entry.logPosition[0], 65536]]
                  }]
                }
                if (entry.lineNumber) {
                  message.range = [
                    [entry.lineNumber - 1, 0],
                    [(entry.endLineNumber ? entry.endLineNumber : entry.lineNumber) - 1, 65536]
                  ]
                }
                return message
              }))
      ...
      try {
        for (const jobname of jobnames) {
          ...
          if (this.linterIndie) {
            createLinterMessages(jobname, 'Warning',
              _.filter(result.warnings,
                (message) => {
                  return (showBuildWarning && (!message.type || message.type === 'Warning')) || (showBuildInfo && message.type === 'Info')
                }))
            createLinterMessages(jobname, 'Error', result.errors)
          }

...
export default class Composer extends Disposable {
  ...
  async build (shouldRebuild) {
    ...
    if (atom.config.get('latex.useOuroboros')) {
      return await this.runOuroboros(filePath, ['load', 'build', 'log', 'save'], { ignoreCache: shouldRebuild }, this.shouldOpenResult(), 'LaTeX Build')
    }
  ...
  async runOuroboros (texFilePath, commands, options = {}, openResults = true, label = null) {
    const ouroboros = await this.getOuroboros(texFilePath, options)
  ...
  async sync () {
    ...
    if (atom.config.get('latex.useOuroboros')) {
      return await this.runOuroboros(filePath, ['load'])
    }
  ...
  async clean () {
    ...
    if (atom.config.get('latex.useOuroboros')) {
      return await this.runOuroboros(filePath, ['load', 'clean', 'save'], {}, false, 'LaTeX Clean')
    }

...
export default class Composer extends Disposable {
  ...
  async build (shouldRebuild) {
    ...
    if (atom.config.get('latex.useOuroboros')) {
      return await this.runOuroboros('LaTeX Build', filePath, ['load', 'build', 'log', 'save'])
    }
  ...
  async runOuroboros (label, filePath, commands) {
    latex.status.show(label, 'highlight', 'sync', true, 'Click to kill LaTeX build.', () => latex.process.killChildProcesses())
    latex.log.group(label)

    const builder = await Builder.create(filePath)
    ...
    builder.on('log', event => {
      ...
      if (event.source) {
        message.filePath = path.resolve(builder.rootPath, event.source.file)
      ...
      if (event.log) {
        message.logPath = path.resolve(builder.rootPath, event.log.file)
        if (event.log.start) {

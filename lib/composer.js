...
import { getEditorDetails, heredoc, promisify, replaceProperitiesInString } from './werkzeug'
...
export default class Composer {
  ...
  async cleanJob (builder, rootFilePath, jobname) {
    ...
    const fdbFiles = _.uniq(_.map(_.flatten(_.values(builder.parseFdbFile(rootFilePath, jobname) || [])), file => path.resolve(dir, file)))
    let files = []

    const properties = {
      output: output ? output + path.sep : output,
    ...
    for (const pattern of cleanPatterns) {
      const replacedPattern = replaceProperitiesInString(pattern, properties)
      const absolutePattern = path.join(dir, replacedPattern)
      if (replacedPattern[0] === path.sep) {
        files = _.uniq(_.concat(files, glob.sync(absolutePattern)))
      } else {
        files = _.uniq(_.concat(files, _.filter(fdbFiles, minimatch.filter(absolutePattern))))
      }
    }

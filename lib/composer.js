...
export default class Composer extends Disposable {
  ...
  constructor () {
    ...
    this.disposables.add(atom.config.onDidChange('latex', () => {
      ...
      // User options for Ouroboros will be updated when the first new builder
      // is created.
  ...
  async getOuroboros (filePath, options = {}) {
    ...
    if (!options.ignoreCache) {
      ouroboros = this.cachedOuroboros.get(filePath)
      if (ouroboros) return ouroboros
    }

    ouroboros = await Ouroboros.create(filePath, options)
    if (this.cachedOuroboros.size === 0) {
      // This is the first Ouroboros builder so make sure the user options
      // are syncronized.
      ouroboros.updateOptions(this.getOuroborosOptions(), true)
    }
    ...
    .on('action', event => {
      ...
      latex.log.info(`[${event.rule}] Evaluating ${event.action} action${triggerText}`)
    })
    .on('command', event => {
  ...
  async build (shouldRebuild) {
    ...
    if (atom.config.get('latex.enableOuroboros')) {
  ...
  async runOuroboros (commands, options = {}, openResults = true, label = null) {
    ...
    if (label) {
      latex.status.show(label, 'highlight', 'sync', true, 'Click to kill LaTeX build.', () => ouroboros.killChildProcesses())
      latex.log.group(label)
    }
    ...
    if (openResults && success) {
      const targets = await ouroboros.getTargetPaths()

...
function renderMessageCount (counts, type) {
  return (
...
export default class StatusLabel {
  ...
  render () {
    ...
    return (
      <div className={classes.join(' ')} onclick={() => latex.log.show()}>
        ...
        <a href='#'>LaTeX</a>
        {renderMessageCount(counts, 'error')}
        {loggingLevel !== 'error' ? renderMessageCount(counts, 'warning') : ''}
        {loggingLevel === 'info' ? renderMessageCount(counts, 'info') : ''}
      </div>
    )
  }
  ...
  readAfterUpdate () {
    if (this.tooltip) {
      ...
      this.disposables.remove(this.tooltip)
      this.tooltop = null
    }

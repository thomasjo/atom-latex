...
import LogMessageView from './log-message' // eslint-disable-line no-unused-vars
  ...
  setProperties (properties: Object) {
    if (properties.messages) {
      this.messages = _.sortBy(properties.messages,
        (message: LogMessage): number => message.range ? message.range[0][0] : -1,
        (message: LogMessage): number => message.logRange ? message.logRange[0][0] : -1
      )
    }

...
function fileReferenceElement (filePath, range, referenceType) {
  const lineReference = range ? <span className='latex-line-reference'>{range[0][0] + 1}</span> : ''
  ...
  if (filePath) {
    return <span className={className}><span className='latex-file-link' onclick={clickHandler}>{text}{lineReference}</span></span>
  }

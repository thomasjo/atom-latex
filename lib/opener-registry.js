...
export default class OpenerRegistry {
  ...
  findOpener (filePath: string): ?Opener {
    ...
    if (enableSynctex) {
      ...
      // the priority sort.
      const opener: ?Opener = rankedCandidates.find((opener: Opener) => opener.hasSynctex())
      if (opener) return opener
    }

    if (openResultInBackground) {
      const opener: ?Opener = rankedCandidates.find((opener: Opener) => opener.canOpenInBackground())
      if (opener) return opener
    }

...
export default class OpenerRegistry extends Disposable {
  ...
  async checkRuntimeDependencies () {
    const pdfOpeners = Array.from((await this.getCandidateOpeners('foo.pdf')).keys())
    ...
    const psOpeners = Array.from((await this.getCandidateOpeners('foo.ps')).keys())
    if (psOpeners.length) {
    ...
    const dviOpeners = Array.from((await this.getCandidateOpeners('foo.dvi')).keys())
  ...
  async findOpener (filePath) {
    ...
    const candidates = Array.from((await this.getCandidateOpeners(filePath)).values())

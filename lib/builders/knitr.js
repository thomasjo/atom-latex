...
export default class KnitrBuilder extends Builder {
  ...
  async run (state) {
    ...
    const { statusCode, stdout, stderr } = await this.execRscript(state.getProjectPath(), args, 'error')
    if (statusCode !== 0) {
      this.logStatusCode(statusCode, stderr)
      return statusCode
    }

    state.setTexFilePath(this.resolveOutputPath(state.getKnitrFilePath(), stdout))
    const builder = latex.builderRegistry.getBuilder(state.getTexFilePath())
    const code = await builder.run(state)

    if (code === 0 && state.getEnableSynctex()) {
      if (state.getOutputDirectory()) {
        ...
        await this.execRscript(state.getProjectPath(), args, 'warning')
      }
    }
  ...
  constructArgs (state) {
    const args = [
      ...
      `-e "knit('${state.getKnitrFilePath().replace(/\\/g, '\\\\')}')"`
    ]
  ...
  constructPatchSynctexArgs (state) {
    const args = [
      ...
      `-e "patchSynctex('${state.getKnitrFilePath().replace(/\\/g, '\\\\')}')"`
    ]

...
export default class LatexmkBuilder extends Builder {
  ...
  async checkAvailability () {
    const { statusCode } = await this.execLatexmk('.', ['-v'], 'error')
  ...
  getPossiblePaths () {
    switch (process.platform) {
      case 'win32':
        return [
          path.join(process.env.SystemDrive, 'texlive\\2016\\bin\\win32'),
          path.join(process.env.SystemDrive, 'texlive\\2015\\bin\\win32'),
          path.join(process.env.SystemDrive, 'texlive\\2014\\bin\\win32'),
          path.join(process.env.ProgramW6432, 'MiKTeX 2.9\\miktex\\bin\\x64'),
          path.join(process.env['ProgramFiles(x86)'], 'MiKTeX 2.9\\miktex\\bin')
        ]
      case 'macOS':
        return [
          '/usr/texbin',

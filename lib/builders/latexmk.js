...
export default class LatexmkBuilder extends Builder {
  ...
  async run (state) {
    ...
    const { statusCode, stderr } = await this.execLatexmk(state.getProjectPath(), args, 'error')
    if (statusCode !== 0) {
  ...
  constructArgs (state) {
    ...
    if (state.getShouldRebuild()) {
      args.push('-g')
    }
    if (state.getJobname()) {
      args.push(`-jobname=${state.getJobname()}`)
    }
    if (state.getEnableShellEscape()) {
      args.push('-shell-escape')
    }
    if (state.getEnableSynctex()) {
      args.push('-synctex=1')
    }
    if (state.getEnableExtendedBuildMode()) {
      const latexmkrcPath = path.resolve(__dirname, '..', '..', 'resources', 'latexmkrc')
      args.push(`-r "${latexmkrcPath}"`)
    }

    if (state.getEngine().match(LATEX_PATTERN)) {
      args.push(`-latex="${state.getEngine()}"`)
      args.push(state.getOutputFormat() === 'pdf'
      ...
      if (state.getOutputFormat() === 'pdf' && state.getEngine().match(PDF_ENGINE_PATTERN)) {
        args.push(`-${state.getEngine()}`)
      } else {
        // Keep the option noise to a minimum by not passing default engine
        if (state.getEngine() !== 'pdflatex') {
          args.push(`-pdflatex="${state.getEngine()}"`)
        }
        args.push(`-${state.getOutputFormat()}`)
      }
    }

    if (state.getOutputDirectory()) {
      args.push(`-outdir="${state.getOutputDirectory()}"`)
    }

    args.push(`"${state.getTexFilePath()}"`)
    return args
  }

  constructPdfProducerArgs (state) {
    switch (state.getProducer()) {
      case 'ps2pdf':
        return '-pdfps'
      case 'dvipdf':
        return '-pdfdvi -e "$dvipdf = \'dvipdf %O %S %D\';"'
      default:
        return `-pdfdvi -e "$dvipdf = '${state.getProducer()} %O -o %D %S';"`
    }
  }
}

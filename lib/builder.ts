...
import { heredoc, isDviFile, isPdfFile, isPsFile } from "./werkzeug";

export default abstract class Builder {

  // TODO: Find a better way of solving the lack of support for static members on interfaces.
  public static canProcess(_state: BuildState) {
    throw new Error("Implementing class must override static function `canProcess`.");
  }
  public executable?: string;
  public envPathKey = this.getEnvironmentPathKey(process.platform);

  public abstract async run(jobState: JobState): Promise<number>;
  public abstract constructArgs(jobState: JobState): string[];
  public abstract async checkRuntimeDependencies(): Promise<void>;
  ...
  public parseLogFile(jobState: JobState) {
  ...
  public getLogParser(logFilePath: string, texFilePath: string) {
    return new LogParser(logFilePath, texFilePath);
  }

  public constructChildProcessOptions(directoryPath: string, defaultEnv?: any) {
  ...
  public resolveOutputFilePath(jobState: JobState, ext: string) {
  ...
  public resolveLogFilePath(jobState: JobState) {
    return this.resolveOutputFilePath(jobState, ".log");
  }

  public getEnvironmentPathKey(platform: string) {
    if (platform === "win32") { return "Path"; }
    return "PATH";
  }

  public resolveFdbFilePath(jobState: JobState) {
    return this.resolveOutputFilePath(jobState, ".fdb_latexmk");
  }

  public parseFdbFile(jobState: JobState) {
  ...
  public getFdbParser(fdbFilePath: string) {
    return new FdbParser(fdbFilePath);
  }

  public parseLogAndFdbFiles(jobState: JobState) {
    ...
    if (fdb) {
      ...
      for (const section of sections) {
        if (fdb[section] && fdb[section].generated) {
          ...
          output = generated.find((file: string) => isPdfFile(file));
          if (output) { break; }

          output = generated.find((file: string) => isPsFile(file));
          if (output) { break; }

          output = generated.find((file: string) => isDviFile(file));
          if (output) { break; }
        }
      }

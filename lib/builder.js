  ...
  parseLogAndFdbFiles (texFilePath, jobname) {
    ...
    if (result) {
      ...
      if (result.fdb) {
        ...
        const output = _.find(outputs, outputPath => path.isAbsolute(outputPath) && path.extname(outputPath).match(OUTPUT_EXTENSION_PATTERN))
        if (output) result.outputFilePath = path.normalize(output)
      }
    }

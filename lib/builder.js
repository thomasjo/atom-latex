...
import SecondaryLogParser from './parsers/secondary-log-parser'
...
export default class Builder {
  ...
  resolveSecondaryLogFilePath (jobState) {
    return this.resolveOutputFilePath(jobState, '.*lg')
  }

  parseSecondaryLogFile (jobState) {
    const logFilePattern = this.resolveSecondaryLogFilePath(jobState)

    for (const filePath of glob.sync(logFilePattern)) {
      const parser = this.getSecondaryLogParser(filePath, jobState.getTexFilePath())
  ...
  getSecondaryLogParser (logFilePath, texFilePath) {
    return new SecondaryLogParser(logFilePath, texFilePath)
  }

  parseLogAndFdbFiles (jobState) {
    this.parseLogFile(jobState)
    this.parseFdbFile(jobState)
    this.parseSecondaryLogFile(jobState)

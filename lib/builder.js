...
export default class Builder {
  ...
  parseLogFile (state) {
    ...
    if (fs.existsSync(logFilePath)) {
      ...
      if (result) {
        state.results.log = result
        if (result.outputFilePath) {
          state.outputFilePath = result.outputFilePath
        }
      }
    }
  }
  ...
  parseLogAndFdbFiles (state) {
    ...
    if (state.results.fdb) {
      const sections = ['ps2pdf', 'dvipdf', 'dvips', 'latex', 'pdflatex']
      let output

      for (const section of sections) {
        if (state.results.fdb[section] && state.results.fdb[section].generated) {
          const generated = state.results.fdb[section].generated
      ...
      if (output) {
        state.outputFilePath = path.resolve(state.projectPath, path.normalize(output))
      }
    }
  }
}

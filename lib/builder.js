...
export default class Builder {
  ...
  parseLogFile (texFilePath, jobname) {
    ...
    const fdbResult = this.parseFdbFile(texFilePath, jobname)
    if (fdbResult) {
      const outputs = fdbResult.ps2pdf || fdbResult.dvipdf ||
        fdbResult.dvips || fdbResult.latex || fdbResult.pdflatex

      const output = _.find(outputs, outputPath => path.isAbsolute(outputPath) && path.extname(outputPath).match(/^\.pdf|\.ps|\.dvi|\.xdv$/))
      if (output) result.outputFilePath = output
    }
  ...
  parseFdbFile (texFilePath, jobname) {
    ...
    const parser = new FdbParser(fdbFilePath)
    return parser.parse()
  }
}

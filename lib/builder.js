...
import BibtexLogParser from './parsers/bibtex-log-parser'
import BiberLogParser from './parsers/biber-log-parser'
import { heredoc, isPdfFile, isPsFile, isDviFile } from './werkzeug.js'

const BIB_LOG_PATTERN = /\.blg$/i
const BIBTEX_SECTION_PATTERN = /^bibtex/
const BIBER_SECTION_PATTERN = /^biber/

function* getGeneratedFilesInSection (projectPath, section, filePathPattern) {
  if (section && section.generated) {
    for (const filePath of section.generated) {
      if (filePath.match(filePathPattern)) {
        yield path.resolve(projectPath, filePath)
      }
    }
  }
}
...
export default class Builder {
  ...
  *getGeneratedFiles (jobState, sectionPattern, filePathPattern) {
    const fileDatabase = jobState.getFileDatabase()
    if (!fileDatabase) return

    if (sectionPattern instanceof RegExp) {
      for (const section in fileDatabase) {
        if (section.match(sectionPattern)) {
          yield * getGeneratedFilesInSection(jobState.getProjectPath(), fileDatabase[section], filePathPattern)
        }
      }
    } else {
      yield * getGeneratedFilesInSection(jobState.getProjectPath(), fileDatabase[sectionPattern], filePathPattern)
    }
  }
  ...
  parseBibtexLogFile (jobState) {
    for (const filePath of this.getGeneratedFiles(jobState, BIBTEX_SECTION_PATTERN, BIB_LOG_PATTERN)) {
      const parser = this.getBibtexLogParser(filePath, jobState.getTexFilePath())
      const logMessages = parser.parse()
      if (logMessages) {
        jobState.addLogMessages(logMessages)
      }
    }
  }

  getBibtexLogParser (logFilePath, texFilePath) {
    return new BibtexLogParser(logFilePath, texFilePath)
  }

  parseBiberLogFile (jobState) {
    for (const filePath of this.getGeneratedFiles(jobState, BIBER_SECTION_PATTERN, BIB_LOG_PATTERN)) {
      const parser = this.getBiberLogParser(filePath, jobState.getTexFilePath())
      const logMessages = parser.parse()
      if (logMessages) {
        jobState.addLogMessages(logMessages)
      }
  ...
  getBiberLogParser (logFilePath, texFilePath) {
    return new BiberLogParser(logFilePath, texFilePath)
  }

  parseLogAndFdbFiles (jobState) {
    this.parseLogFile(jobState)
    this.parseFdbFile(jobState)
    this.parseBibtexLogFile(jobState)
    this.parseBiberLogFile(jobState)

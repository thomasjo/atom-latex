...
export default {
  heredoc (input: ?string): ?string {
    ...
    const lines: Array<string> = _.dropWhile(input.split(/\r\n|\n|\r/), line => line.length === 0)
    const indentLength: number = _.takeWhile(lines[0], char => char === ' ').length
    const truncatedLines: Array<string> = lines.map(line => line.slice(indentLength))

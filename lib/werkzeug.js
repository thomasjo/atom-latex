...
import type { EditorDetails, UnaryFunction } from './types'

const INDENT_PATTERN: RegExp = /^[\n\r]*( +)/
...
export default {
  ...
  replacePropertiesInString (text: string, properties: Object): string {
    return _.reduce(properties, (current: string, value: string, name: any) => current.replace(`{${name}}`, value), text)
  },

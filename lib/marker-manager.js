...
export default class MarkerManager extends Disposable {
  ...
  constructor (editor) {
    ...
    const clearListener = () => this.clear()
    const messagesListener = messages => this.addMarkers(messages)

    latex.log.on('clear', clearListener)
    latex.log.on('messages', messagesListener)

    this.disposables.add(new Disposable(() => {
      this.clear()
      latex.log.removeListener('clear', clearListener)

...
export default class Logger {
  constructor () {
    this.messages = []
    this._group = null
    const callback = () => { this.showFilteredMessages() }
    atom.config.onDidChange('latex.showBuildWarning', callback)
    atom.config.onDidChange('latex.showBuildInfo', callback)
  }
  ...
  groupEnd () {
    this.messages = _.sortBy(this._group, 'filePath', (message) => { return message.range ? message.range[0][0] : -1 })
    this._group = null

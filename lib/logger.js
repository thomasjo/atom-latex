...
export default class Logger extends Disposable {
  ...
  showMessage (message) {
    ...
    if (this.isMessageTypeVisible(message.type)) {
  ...
  getMessages (useFilters = true) {
    return useFilters
      ? this.messages.filter(message => this.isMessageTypeVisible(message.type))
      : this.messages
  }
  ...
  async sync () {
    ...
    if (filePath) {
      const logDock = await atom.workspace.open(LogDock.LOG_DOCK_URI)
      if (logDock) {
  ...
  toggle () {
    atom.workspace.toggle(LogDock.LOG_DOCK_URI)
  }

  show () {
    atom.workspace.open(LogDock.LOG_DOCK_URI)
  }
